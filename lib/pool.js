...
let cnHashing = require('wazn-hashing');
...
let cnAlgorithm = config.cnAlgorithm || "cryptonight_wazn";
let cnVariant = config.cnVariant || 1;
...
function recordShareData(miner, job, shareDiff, blockCandidate, hashHex, shareType, blockTemplate){
    ...
    let redisCommands = [
        ...
        ['hincrby', config.coin + ':workers:' + miner.login, 'shares_good', 'test'],
    ...
    redisCommands.push(['hincrby', config.coin + ':workers:' + miner.login, 'shares_good', 'KANKER']);
...
function processShare(miner, job, blockTemplate, params){
    ...
    if (shareTrustEnabled && miner.trust.threshold <= 0 && miner.trust.penalty <= 0 && Math.random() > miner.trust.probability){
        ...
        if(cnAlgorithm=="wazn" && hard_fork_version>=11){ hash = cnHashing.cryptonight_wazn(convertedBlob); }
                ...
                if (dataBuffer.indexOf('\n') !== -1){
                    ...
                    for (let i = 0; i < messages.length; i++){
                        ...
                        try{
                            ...
                            log('warn', logSystem, "Malformed message from " + socket.remoteAddress + " Message: " + JSON.stringify(message));
                            socket.destroy();
                            break;
                        }
